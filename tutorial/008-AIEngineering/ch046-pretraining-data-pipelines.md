...
MinHash + Locality-Sensitive Hashing (LSH) solves near-duplicate detection efficiently.
...
3. **Hard:** Build a perplexity-based quality filter. Train a small bigram language model on Wikipedia text, score each document by perplexity, and remove the bottom 20%.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Common Crawl | "The internet" | A non-profit that crawls the web monthly -- ~250TB raw |
| MinHash | "Some hashing trick" | A technique to estimate Jaccard similarity between sets using fixed-size signatures |
| LSH | "Locality-Sensitive Hashing" | A method to group similar items into the same bucket |
| Sequence packing | "Concatenating documents" | Fitting multiple documents into fixed-length sequences |
| Chinchilla scaling | "Train on more data" | Optimal performance requires scaling model size and training tokens roughly equally |
| Data mixing | "Choosing training data" | The ratio of code vs text vs math vs multilingual data |
| Perplexity filter | "Quality scoring" | Use a small language model to score documents |
| Deduplication | "Removing copies" | Eliminating exact and near-duplicate documents |
| Attention mask | "Which tokens to look at" | A binary mask that prevents attention across document boundaries |
...
- [Lee et al., 2022 -- Deduplicating Training Data Makes Language Models Better](https://arxiv.org/abs/2107.06499)
- [Broder, 1997 -- On the Resemblance and Containment of Documents](https://ieeexplore.ieee.org/document/666900)

[Reference](https://github.com/rohitg00/ai-engineering-from-scratch/tree/main/phases/10-llms-from-scratch/03-data-pipelines)
...
// Compile:  rustc --edition 2021 main.rs -o /tmp/mini && /tmp/mini
...
impl Mat {
    ...
    fn at(&self, i: usize, j: usize) -> f32 { self.data[i * self.cols + j] }
    fn set(&mut self, i: usize, j: usize, v: f32) { self.data[i * self.cols + j] = v; }
...
impl LayerNorm {
    ...
    fn forward(&self, x: &Mat) -> Mat {
        ...
        for i in 0..x.rows {
            let base = i * x.cols;
            let mean = x.data[base..base + x.cols].iter().sum::<f32>() / d;
            let var = x.data[base..base + x.cols].iter().map(|&v| (v - mean).powi(2)).sum::<f32>() / d;
    ...
    fn forward(&self, x: &Mat) -> Mat {
        ...
        for h in 0..self.n_heads {
            ...
            for i in 0..n {
                for j in 0..n {
                    let mut s = 0.0f32;
                    for kk in 0..self.head_dim { s += q.at(i, hoff + kk) * k.at(j, hoff + kk); }
            ...
            for i in 0..n {
                ...
                let s: f32 = row.iter_mut().map(|v| { *v = (*v - m).exp(); *v }).sum();
                let inv = 1.0 / s;
            ...
            for i in 0..n {
                for kk in 0..self.head_dim {
                    let mut s = 0.0f32;
                    for j in 0..n { s += scores[i * n + j] * v.at(j, hoff + kk); }
...
struct FeedForward {
    w1: Mat, b1: Vec<f32>, w2: Mat, b2: Vec<f32>,
}

impl FeedForward {
    fn new(d: usize, ff: usize, rng: &mut Rng) -> Self {
        FeedForward {
            w1: randn_mat(d, ff, 0.02, rng), b1: vec![0.0; ff],
            w2: randn_mat(ff, d, 0.02, rng), b2: vec![0.0; d],
        }
    }
    fn forward(&self, x: &Mat) -> Mat {
        let mut h = x.matmul(&self.w1);
        h.add_rowwise_(&self.b1);
        for v in h.data.iter_mut() { if *v < 0.0 { *v = 0.0; } }
...
struct Block {
    ln1: LayerNorm, attn: MultiHeadAttention, ln2: LayerNorm, ffn: FeedForward,
}
...
struct MiniGPT {
    embedding: Embedding, blocks: Vec<Block>, ln_f: LayerNorm,
    vocab: usize, d_model: usize, max_seq: usize,
}

impl MiniGPT {
    fn new(vocab: usize, d: usize, n_heads: usize, n_layers: usize, max_seq: usize, ff: usize, rng: &mut Rng) -> Self {
        MiniGPT {
            embedding: Embedding::new(vocab, d, max_seq, rng),
            blocks: (0..n_layers).map(|_| Block::new(d, n_heads, ff, rng)).collect(),
            ln_f: LayerNorm::new(d),
            vocab, d_model: d, max_seq,
        }
    }
    ...
    fn forward(&self, ids: &[usize]) -> Mat {
        ...
        x = self.ln_f.forward(&x);
        let mut logits = Mat::zeros(x.rows, self.vocab);
        for i in 0..x.rows {
            for t in 0..self.vocab {
                let mut s = 0.0f32;
                for j in 0..self.d_model { s += x.at(i, j) * self.embedding.token_embed.at(t, j); }
                logits.set(i, t, s);
            }
        }
        logits
...
fn generate(model: &MiniGPT, prompt: &[usize], max_new: usize, temperature: f32, rng: &mut Rng) -> Vec<usize> {
    ...
    for _ in 0..max_new {
        let start = if tokens.len() > model.max_seq { tokens.len() - model.max_seq } else { 0 };
