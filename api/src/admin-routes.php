...
function verifyAdmin(Request $request): bool {
    ...
    try {
        JWT::decode(substr($header, 7), new Key($_ENV['JWT_SECRET'] ?? '', 'HS256'));
        return true;
    } catch (\Exception $e) {
        return false;
    }
...
return function (App $app, ?PDO $pdo) {
    ...
    $app->post('/admin/sync-blogs', function (Request $request, Response $response) use ($pdo) {
        if (!$pdo) {
        ...
        if (empty($files)) {
            return jsonResponse($response, ['error' => 'No markdown files found'], 500);
        }
        ...
        if ($ids) {
            ...
            $pdo->prepare("UPDATE blog SET deleted = 1 WHERE id NOT IN ({$placeholders})")->execute($ids);
        } else {
            $pdo->exec('UPDATE blog SET deleted = 1');
        }

        return jsonResponse($response, ['message' => "Synced {$parsed} blog posts"]);
    });
};
